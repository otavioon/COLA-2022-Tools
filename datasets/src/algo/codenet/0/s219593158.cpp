...
ll solve(string s, int k) {
    ...
    int pre = -1;
    REP(i,n) {
      if (i>=1&&s[i]==s[i-1]&&pre!=i-1) cnt++,pre=i;
    }
    if (k == 1) {
      return cnt;
    }
    ...
    s += s;
    for (int i=n; i<2*n; i++) {
      if (s[i]==s[i-1]&&pre!=i-1) cnt2++,pre=i;
    }
    //cout << "!" << " " << pre << endl;
    if (pre==2*n-1) {
      return (ll)(cnt+cnt2) * (k/2) + (k%2) * cnt;
    } else {
      return cnt + (ll)cnt2 * (k-1);
    }
}
int main() {
  string s;
  ...
  while(cin>>s>>k) {
    ...
    // int pre = -1;
    // REP(i,t.size()) {
    //   if (i>=1&&t[i]==t[i-1]&&pre!=i-1) cnt++,pre=i;
    // }
    // if (ans != cnt) cout << s << " " << k << " " << tpl(ans,cnt) <<endl;
    // }
  }
}

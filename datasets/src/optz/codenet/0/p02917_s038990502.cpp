...
int32_t main() {
    ...
    int ans=0;
    for(int i=0;i<n-2;i++) 
    ans+=(min(a[i],a[i+1]));                /// this question is done like this bcoz we have Bi >= max(ai,ai+1)
                                            /// thus to find ai we need to focus that what can be the min value at ai
                                            /// so that both statements contradicts and we will get the = max value for ai

...
void assigndepth(Veci* children,int which, int* depth, int n)
{
	if (depth[which] == -1) return;
	
	repi(children[which].n)
	{
		int k = children[which].data[i];
		if (depth[k] == -1)
		{
			depth[k] = depth[which] + 1;
			assigndepth(children, k, depth, n);
		}
	}
}
...
int main()
{
	...
	static int depth[2 * ten5p1];
	static int q[2 * ten5p1];
	zero(q, nvert);
	repi(nvert)depth[i] = -1;
	zero(counter, nvert);
	...
	depth[0] = 0;
	assigndepth(children, 0, depth, nvert);
	...
	repi(nvert)
	{
		...
		repj(children[i].n)
		{
			if (depth[children[i].data[j]] <= depth[i]) continue;

...
int main()
{
	...
	static char blue[100][101];
	static char red[100][101];

	int nblue = readi();
	repi(nblue) reads(blue[i], 101);
	int nred = readi();
	repi(nred) reads(red[i], 101);
	int maxearn = 0;
	repi(nblue)
	{
		int nb = 0;
		repj(nblue)
		{
			if (strcmp(blue[i], blue[j]) == 0) nb++;
		}
		int nr = 0;
		repj(nred)
		{
			if (strcmp(blue[i], red[j]) == 0) nr++;
		}
		if (nb > nr)
		{
			int earn = nb - nr;

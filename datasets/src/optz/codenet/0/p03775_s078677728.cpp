...
void solve(void){
    ...
    rep(n, i){
        ...
        int index = 0;
        rep(m, j) {
            if (man_len > abs(checkpoints[j].first - students[i].first) + abs(checkpoints[j].second - students[i].second)) {
                man_len = abs(checkpoints[j].first - students[i].first) + abs(checkpoints[j].second - students[i].second);
                index = j;
            }
        }
        cout << index+1;

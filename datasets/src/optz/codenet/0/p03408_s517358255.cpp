...
// list declaration
VarList list_s({Var("red"), Var("red"), Var("blue"), Var("yellow"), Var("yellow"), Var("red")});
VarList list_t({Var("red"), Var("red"), Var("yellow"), Var("green"), Var("blue")});
VarList list_index({Var("red"), Var("blue"), Var("yellow")});
VarList list_value({Var(1), Var(0), Var(1)});
...
int main(){
    ...
    cin >> buf_answer;
    var_N = buf_answer;
    for (int plywgshh=0;plywgshh<var_N.asNumber();plywgshh++){
        cin >> buf_answer;
        list_s.push_back(buf_answer);
        if ((Var(list_index.itemNumOfList(buf_answer)) == Var(0))){
            list_index.push_back(buf_answer);
            list_value.push_back(Var(1));
        } else {
            list_value.setLineOfListTo(Var(list_index.itemNumOfList(buf_answer)), (list_value.getLineOfList(Var(list_index.itemNumOfList(buf_answer))) + Var(1)));
        }
    }
    cin >> buf_answer;
    var_M = buf_answer;
    for (int pqdizozg=0;pqdizozg<var_M.asNumber();pqdizozg++){
        cin >> buf_answer;
        list_t.push_back(buf_answer);
        /* ERROR: control_if_else */
    }
    var_ans = Var(0);
    var_i = Var(1);
    for (int xkvkoxuo=0;xkvkoxuo<Var(list_value.size()).asNumber();xkvkoxuo++){
        if ((list_value.getLineOfList(var_i) > var_ans)){
            var_ans = list_value.getLineOfList(var_i);
        }
        var_i = (var_i + Var(1));
    }
    func_main();

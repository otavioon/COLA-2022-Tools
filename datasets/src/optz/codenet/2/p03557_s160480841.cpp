...
ll bs(vector<ll> v,ll key){ //key以上が初めて出てくる添字(0-index)
    int l=0,r=sz(v)-1;
    while(r-l>=1){
        int i=(l+r)/2;
        //cout<<l<<" "<<r<<" "<<i<<endl;
...
int main(){
    ...
    rep1(i,N){
        ll tmp=B[i]; //真ん中さいず
        ll c_check=bs(C,tmp+1);
        if(c_check==INF) continue;
        ll c_size=N-c_check+1;
        
        ll a_check=bs(A,tmp);
        ll a_size=0;
        if(a_check==INF) a_size=N;
        else a_size=a_check-1;

...
int n,k,d[14],ans,dp[10][20][2],sm;
bool dame[14];
string s;
signed main() {
    ...
    ans+=mypow(10, keta(n))*sm;
    rep(i,keta(n)){
        rep(big,2){
            RREP(j,(big?0:s[i]-'0'),9){
                if (dame[j]) {
                    continue;
                }
                rep(k,10){
                    chmin(dp[i+1][j][(big||j<s[i]-'0'?1:0)],dp[i][k][big]*10+j);
                }
            }
        }
    }
